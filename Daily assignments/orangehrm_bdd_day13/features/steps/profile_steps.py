...
@when('the user updates the nickname to "{nickname}"')
def step_update_nickname(context, nickname):
    context.my_info_page.update_nickname(nickname)


@when('the user clicks the save button on personal details')
...
@when('the user uploads a profile photograph "{filename}"')
